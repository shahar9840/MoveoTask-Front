...
function Player({token}) {
    ...
    const isTitleHebrew = presentSong?.title && containsHebrew(presentSong.title);
      ...
      const isSinger = () => {
          axios.get("http://localhost:50000/is_singer", {
              headers: {
                Authorization: `Bearer ${token}`,
              },
          }).then((response) => {
              console.log("Response:", response);
              if (response.data.is_singer) {
                return true
              }
          })
      }
...
{presentSong!== null && presentSong !== "" && presentSong !== undefined ? <div>
    <div style={{
              display: "flex",
    ...
    {presentSong.lyrics.map((verse, index) => (
            ...
            ))}</div> : <h1>Waitng for the next song{dots}</h1>}
